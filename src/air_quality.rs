...
/// `plugins::air_quality` instead of a single "now" figure, which sidesteps
/// needing one anyway. No per-point timestamp: `forecast_days=1` always
/// starts at local midnight, so array index *is* hour-of-day directly.
#[derive(Deserialize, Debug, Clone)]
pub struct Hourly {
